...
void removechar(const vector<platechar> &vectorofchar,vector<platechar> &vectorofcharo){
	for (int i = 0; i < vectorofchar.size()-1; i++)
	{
	if (distanceBetweenChars(vectorofchar[i], vectorofchar[i + 1])<90){
		std::cout << distanceBetweenChars(vectorofchar[i], vectorofchar[i + 1]) << endl;
			vectorofcharo.push_back(vectorofchar[i]);
		}
	else{
		vectorofcharo.push_back(vectorofchar[i]);
	}
...
void drawchars(cv::Mat &pic, vector<platechar> &vectorOfChars, cv::Scalar &color){
	...
	for (int i = 0; i < vectorOfChars.size(); i++){
		drawContours(pic, contra, i, color, CV_FILLED, 8, hierarchy, 0, cv::Point());
		rectangle(pic, vectorOfChars[i].boundrect, cv::Scalar(0, 0, 255), 1, 8, 0);
...
vector<platechar> possibleContours(cv::Mat &image, vector<platechar> &allcontours){
	...
	cv::Mat grayscale;
	cv::Mat origin = preprocess(image, grayscale);
	cv::Mat imagecopy = origin.clone();
	...
	for (int i = 0; i< contours.size(); i++)
	{
		...
		if (posschar.rectarea>100 && posschar.dblAspectRatio > 0.08 && posschar.dblAspectRatio <.9 &&  posschar.rectarea < 2500 && posschar.boundrect.height>5)
		{
			test.push_back(posschar);
			//cout << test.size();
			//Mat ROI = pic(posschar.boundrect);
			//drawContours(dst, contours, i, color, CV_FILLED, 8, hierarchy, 0, Point());
			//resize(ROI, ROI, Size(0, 0), 2, 2, INTER_LINEAR);
			//cvtColor(ROI, ROI, CV_BGR2GRAY);
			//Canny(ROI, ROI, 50, 150, 3);
			//sprintf(Name, "win %d", c);
			//cv::namedWindow(Name,CV_WINDOW_AUTOSIZE);
			//rectangle(dst,posschar.boundrect,Scalar(0,0,255),1,8,0);
			//cv::imshow(Name, ROI);
		}
	...
	drawchars(dst, test, cv::Scalar(0, 0, 0));
	imshow("asdas", dst);
	if (test.size() < 4){
		std::cout << "SORRY! not able to detect";
		cv::waitKey(5000);
		exit(1);
	}
	return test;
...
////////////////////////////////////////////////////////////////////////////
cv::Mat filterprocess(cv::Mat &pic, vector<platechar> &charsInPlate){
	vector<platechar> vectorofallmatch,testchars;
	vector<platechar> garbage, allchar;
	vector<platechar> test = possibleContours(pic, allchar);
	cv::Mat crop, grayscale;
	cv::Mat pic1 = pic.clone();
    cv::Mat dst = cv::Mat::zeros(pic.size(), CV_8UC3);
	cv::Mat origin = preprocess(pic1, grayscale);
	...
	for (auto &vectorofmatch : VectorOfVects){
		...
		if (vectorofmatch.size() > size ){
			size = vectorofmatch.size();
			vectCount = i;
         }
		i++;
        }
   testchars = VectorOfVects[vectCount];
   charsInPlate = VectorOfVects[vectCount];
	std::sort(testchars.begin(), testchars.end(), platechar::sortCharsLeftToRight);
	//removechar(testchars, charsInPlate);
	cout << "after filter::" << charsInPlate.size() << endl;
	cout << "vectcount " << vectCount << "size " << VectorOfVects.size() <<endl;
	cv::Mat rotationMatrix = plates(origin, pic, charsInPlate);
	imshow("null", pic);
	warpAffine(dst, dst, rotationMatrix, dst.size());///rotate the image before cropping;
	drawchars(dst, charsInPlate, cv::Scalar(255, 255, 255));
	drawchars(dst, VectorOfVectsgarbage[vectCount], cv::Scalar(0, 0, 0));
	//garbage = removeInnerOverlappingChars(vectorofmatch, allchar);
...
cv::Mat plates(cv::Mat &imgOriginal,cv::Mat &imgdrawon, std::vector<platechar> &vectorOfMatchingChars) {
	...
	for (int j = 0; j < 4; j++) {
		cv::line(imgdrawon, p2fRectPoints[j], p2fRectPoints[(j + 1) % 4],cv::Scalar(0, 0, 255), 4);
	}
...
cv::Mat preprocess(cv::Mat &imgOriginal, cv::Mat &imgGrayscale) {
	...
	cv::Mat dilateElement = getStructuringElement(cv::MORPH_RECT, cv::Size(1, 1));
	cv::Mat imgBlurred;
...
cv::Mat maximizeContrast(cv::Mat &imgGrayscale) {
	...
	imshow("blackhat_top", imgGrayscalePlusTopHatMinusBlackHat);
